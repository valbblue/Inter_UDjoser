...
const API_BASE_URL = "http://127.0.0.1:8000"; // ✅ Eliminé el espacio extra al final
...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // ✅ CORREGIDO: Redirigir a PROFILE no a HOME
      window.setTimeout(() => {
        navigate("/profile", { replace: true }); // <-- RUTA CORRECTA
      }, 2000);
    ...
    <div className="bg-linear-to-br from-slate-900 via-purple-900 to-slate-900 min-h-screen flex flex-col relative overflow-x-hidden">
      ...
      <header className={`relative z-10 ${styles.glassEffect}`}>
        <div className="max-w-7xl mx-auto px-4 py-4 border-b border-purple-500/20">
          <div className="flex items-center justify-between">
            ...
            <a href="/register" className="text-slate-300 hover:text-primary transition-colors">Crear Cuenta</a>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 flex items-center justify-center py-12 px-4 relative z-10">
        <div className={`max-w-md w-full ${styles.fadeInUp}`}>
          ...
          {alert && (
            <div className={`mb-4 px-4 py-3 rounded-lg border text-sm ${
              ...
              {DEBUG_MODE && alert.details && (
                <pre className="mt-2 text-xs bg-black/20 p-2 rounded overflow-auto max-h-40 whitespace-pre-wrap">
          ...
          {/* DEBUG INFO SOLO EN CONSOLA, NO EN UI */}
          {DEBUG_MODE && (
            <div className="mt-4 p-3 bg-slate-800/50 rounded-lg border border-purple-500/20">
              <p className="text-xs text-slate-400 font-mono">DEBUG: email={email} | password={password ? '***' : ''}</p>
            </div>
          )}
